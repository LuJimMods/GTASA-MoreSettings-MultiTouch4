...
/* JPatch config: net.rusjj.jpatch.ini */
Config* jpatchCfg = nullptr;
ConfigEntry* pCfgJPatchMultiTouch4 = nullptr;
...
void JPatchMultiTouch4Changed(int oldVal, int newVal)
{
    if(pCfgJPatchMultiTouch4 == nullptr || jpatchCfg == nullptr) return;

    pCfgJPatchMultiTouch4->SetBool(newVal == 0 ? false : true);
    jpatchCfg->Save();

    // JPatch usually reads this value on mod/game start.
    // If it does not apply instantly, restart the game after changing it.
}
...
extern "C" void OnModLoad()
{
    ...
    if(sautils != nullptr)
    {
        ...
        *(bool*)(pGTASA + 0x98F1AD) = pCfgDebugFPS->GetBool();
        ...
        // JPatch MultiTouch4 toggle. Saves to net.rusjj.jpatch.ini [Gameplay] MultiTouch4=0/1
        jpatchCfg = new Config("net.rusjj.jpatch");
        jpatchCfg->Init();
        pCfgJPatchMultiTouch4 = jpatchCfg->Bind("MultiTouch4", false, "Gameplay");
        sautils->AddClickableItem(Game, "MultiTouch 4", pCfgJPatchMultiTouch4->GetInt(), 0, sizeofA(pYesNo)-1, pYesNo, JPatchMultiTouch4Changed);
